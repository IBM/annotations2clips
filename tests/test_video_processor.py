...
def test_check_valid_annotation_file(video_processor) -> None:
    ...
    file_path = Path("tests", "test_data", "station_1", "user_1", "fef69bac-bc6d-47c7-8c8a-bd56ffa0c851.json")
...
def test_get_annotations(video_processor) -> None:
    ...
    number_of_events = len([event for video in processor.annotations.keys() for key in processor.annotations[video].keys() for event in processor.annotations[video][key]])
    unique_actions = len(set([action for video in processor.annotations.keys() for action in processor.annotations[video].keys()]))
...
def test_create_clips_and_metadata_file_creation(video_processor) -> None:
    ...
    assert num_clips == 100
    assert jsonl_lines[0] == ['{"image:":"573ce385-0f6f-4c6b-8730-8130424f7c6f_blue_000_000.mp4"', 'label:"blue"}']
    assert jsonl_lines[-1] == ['{"image:":"fef69bac-bc6d-47c7-8c8a-bd56ffa0c851_yellow_000_001.mp4"', 'label:"yellow"}']
    assert stats_dict == {"blue": 4, "green": 5, "grey": 62, "red": 12, "violet": 18, "white": 35, "indigo": 4, "orange": 2, "yellow": 2}
    assert mapping_dict["573ce385-0f6f-4c6b-8730-8130424f7c6f"] == "tests/test_data/station_1/user_2/573ce385-0f6f-4c6b-8730-8130424f7c6f"
